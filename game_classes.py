...
from image_data import agent, barrier, bomb_exploding, bomb, bullet, cannon, enemy, heart, lightning, poison, shield, zombie
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, target, allies, game):
        super().__init__()
    ...
    def update(self, character, bullets, hearts, shields, scoreboard, allies):
        ...
        if bullet_collisions:
            ...
            if self.game.infected:
                ally = Ally(self.game.enemy_location)
                allies.add(ally)
            
            # Update the variable with the enemy's current location
            self.game.enemy_location = (self.rect.x, self.rect.y)
